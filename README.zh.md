...
### 5. 会话密码缓存
- 开启 **记住密码** 后，最近使用的密码会被自动缓存（按笔记或文件夹区分），直到 Obsidian 关闭或超过设定时长。
- **清除会话密码缓存** 命令可立即清空缓存。
...
| **记住密码（时长）** | 显示当前缓存保留时长，并提供滑块（0–120 分钟）。`0` 表示 Obsidian 关闭时清空缓存。 |
| **行内加密 → 将选区扩展到整行？** | 部分选区在加密前自动扩展为整行。 |
| **行内加密 → 标记搜索范围** | 加密 / 解密时查找标记的最大范围。 |
| **行内加密 → 默认在阅读时显示加密标记** | 行内加密是否在阅读视图中保留可见标记。 |
| **生成随机密码 → 默认长度** | 生成密码的默认字符数。 |
| **生成随机密码 → 包含大写字母（A–Z）** | 生成的密码中包含大写字母。 |
| **生成随机密码 → 包含小写字母（a–z）** | 生成的密码中包含小写字母。 |
| **生成随机密码 → 包含数字（0–9）** | 生成的密码中包含数字。 |
| **生成随机密码 → 包含符号（!@#$...）** | 生成的密码中包含符号。 |
